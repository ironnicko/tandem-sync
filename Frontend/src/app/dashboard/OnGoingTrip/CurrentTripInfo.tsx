...
export default function CurrentTripInfo({ ride }: CurrentTripInfoProps) {
  ...
  const formatTime = (date: string | null | undefined) => {
    if (!date) return "N/A";
    return new Date(date).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", day: "2-digit", month: "2-digit", year: "2-digit" });
  };
  ...
  return (
    <div className="bg-white rounded-lg p-4 shadow-md border border-gray-200">
      ...
      <div className="space-y-2 text-sm">
        ...
        <div className="flex items-center gap-2">
          <MapPin className="w-4 h-4 text-red-600" />
          <span className="font-medium">{ride.destinationName || "Unknown"}</span>
        </div>
